...
function read() {
  ...
  return {
    ...
    preferences: String(values.get('preferences') || '').trim()
  };
}
  ...
  document.querySelectorAll('.rate-work').forEach(button => {
    button.onclick = async () => {
      ...
      try {
        ...
        workCard.querySelector('.work-rating').textContent = `★ ${Number(updated.rating_average).toFixed(1)} · ${updated.rating_count} оценок`;
        workCard.querySelectorAll('.rate-work').forEach(item => {
