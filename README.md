...
Скриншот фирменного визуального направления Rio: [rio-ui-reference.png](docs/screenshots/rio-ui-reference.png). Фактические кадры сценариев 01–06 сохраняются в этой же папке после запуска `python app.py`; в подписи к каждому кадру указываются входные параметры и ожидаемый исход.
