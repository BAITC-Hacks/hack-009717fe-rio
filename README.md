...
**Айназ Абитай** отвечает за качество данных и воспроизводимость демо: проверяет уникальность ID, цены, даты и обязательные поля; запускает `python tools/audit_data.py --include-synthetic --markdown`; контролирует отдельный слой `data/synthetic_profiles.csv`; проверяет сценарии редкой категории, смены даты и пустой выдачи; обновляет README и объясняет жюри происхождение профилей.
