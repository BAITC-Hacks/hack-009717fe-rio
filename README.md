...
> **За полминуты:** Rio превращает длинный каталог event-подрядчиков в короткий проверяемый выбор. Пользователь указывает город, дату, формат, категорию и бюджет, а сервис возвращает до трёх доступных вариантов. На каждой карточке видно, какие условия выполнены, откуда взята цитата и почему другие профили исключены.

## Цель и решаемая проблема

Заказчик уже получил каталог подрядчиков своего города, но не должен вручную просматривать десятки профилей и сверять занятость, цену и формат. Rio решает эту задачу за один запрос:

- оставляет только подрядчиков из выбранного города и категории;
- исключает занятых на нужную дату и тех, кто не проходит по бюджету, формату, языку или длительности;
- ранжирует оставшихся по совпадению с пожеланиями, затем по цене и стабильному ID;
- показывает максимум три карточки с фактами и цитатой из исходного профиля;
- объясняет не только успех, но и короткий список, отсутствие категории и пустой результат.

**Цель проекта** — сделать выбор быстрым, понятным и воспроизводимым, сохранив связь каждой рекомендации с данными каталога. Это не бронирование и не генерация обещаний: сервис помогает принять решение на основе проверяемых условий.

## Что получает жюри при проверке

1. Работающий русскоязычный сайт с готовыми сценариями 01–06.
2. Детерминированную выдачу: одинаковый запрос даёт одинаковый порядок карточек.
3. Объяснение каждой рекомендации и аудит причин исключения альтернатив.
4. Разные состояния продукта: `matched`, `no_category` и `no_match`.
5. Локальный запуск без ключей, базы данных и внешних AI-сервисов.
...
Локальный адрес `localhost` — демо на компьютере, где запущен сервер. Для быстрого просмотра доступна опубликованная версия ниже.

Опубликованная демонстрация команды: [rio-hackalem-live.vercel.app](https://rio-hackalem-live.vercel.app/).

## Быстрый путь проверки

После запуска откройте сайт и нажмите одну из кнопок готового сценария над блоком результатов. Для защиты рекомендуем такой порядок:

1. **01 · Осенний корпоратив** — убедиться, что показаны три карточки, цена, доступность и объяснение.
2. **02 · Другая дата** — увидеть изменение выдачи из-за занятости календаря.
3. **03 · Редкая категория** — проверить, что сервис честно показывает меньше трёх вариантов.
4. **04 · Бюджет не проходит** — увидеть `no_match` и причины исключения.
5. **05 · Нет категории** — увидеть отдельное состояние `no_category`.
6. **06 · Банкетный зал** — убедиться, что площадки проходят тот же конвейер ограничений.

Повторите любой сценарий дважды: порядок карточек и текстовые причины должны остаться одинаковыми.
...
Подробные README участников: [Темирлан — frontend](docs/README.temirlan.md), [Мирас — backend](docs/README.miras.md), [Айназ — данные и демо](docs/README.ainaz.md).
...
Запустить проверку из корня репозитория:
...
Ожидаемый результат — **10/10 тестов со статусом `OK`** и успешный аудит каталога без ошибок. Для ручной проверки API:
...
В ответе `/api/recommend` проверьте поля `status`, `cards`, `pool_count`, `eligible_count`, `rejected`, `exclusion_counts` и `elapsed_ms`. В успешном сценарии `status` равен `matched`, карточек не больше трёх, а объяснение каждой карточки содержит факты запроса и цитату профиля.
...
## Чек-листы участников

- [Чек-лист Темирлана — frontend и UI](docs/README.temirlan.md): запуск сайта, проверка формы, карточек, адаптивности и сценариев демо.
- [Чек-лист Мираса — backend и API](docs/README.miras.md): запуск сервера, проверка endpoint, фильтров, состояний ответа и детерминированности.
- [Чек-лист Айназ — данные и демо](docs/README.ainaz.md): аудит каталога, синтетические профили, тесты и воспроизводимость.
